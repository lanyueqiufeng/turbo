...
public class BranchMergeJoinAll extends BranchMergeStrategy {
    ...
    public void joinMerge(RuntimeContext runtimeContext, NodeInstancePO joinNodeInstancePo, NodeInstanceBO currentNodeInstance,
        ...
        // 1. 获取当前分支数据
        InstanceDataPO joinInstanceData = instanceDataDAO.select(runtimeContext.getFlowInstanceId(),
                runtimeContext.getInstanceDataId());

        // 2. 获取已合并的数据（之前所有分支的合并结果）  joinNodeInstancePo  汇聚节点
        InstanceDataPO mergedData = instanceDataDAO.select(runtimeContext.getFlowInstanceId(),
                joinNodeInstancePo.getInstanceDataId());
        if (mergedData == null) {
        ...
        // 4. 根据是否所有分支都到达，决定完成还是等待 allExecuteIdSet全部分支   arrivedExecuteIds 到达的分支
...
//        instanceDataDAO.updateData(mergedData);
        ...
        // 7. 如果不是所有分支都到达，则挂起
        if (!allArrived) {
            throw new SuspendException(ParallelErrorEnum.WAITING_SUSPEND.getErrNo(),
                    MessageFormat.format(Constants.NODE_INSTANCE_FORMAT,
                            runtimeContext.getCurrentNodeModel().getKey(),
                            runtimeContext.getCurrentNodeModel().getProperties().getOrDefault(
                                    Constants.ELEMENT_PROPERTIES.NAME, StringUtils.EMPTY),
